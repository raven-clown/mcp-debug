...
- Fixed: a topic whose log file couldn't be created used to crash the whole run; it now falls back to the main log with a warning.
- Fixed: logging with a new topic every time (e.g. one per request) could open unlimited files. Now capped at 50 topics per run.
- Fixed: topics differing only in case (`"API"` vs `"api"`) could silently share one folder on Windows/macOS. Topic names are now lowercased consistently.
