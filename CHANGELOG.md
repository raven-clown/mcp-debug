...
- Fixed a crash: if a topic's log file couldn't be set up (its directory failed to create, for example because `MCP_DEBUG_TOPIC_DIR_<TOPIC>` points somewhere unwritable, or the path collides with an existing file), the resulting exception was uncaught and crashed the entire `mcp-debug run` process mid-session, killing the relay for the wrapped MCP server over what should have only affect that one topic's logging. Reproduced by pre-creating a plain file where a topic's directory needed to go, confirmed the crash, then fixed by falling back to the main session log with a warning instead. The same class of failure in `SessionLog.write()` itself (a rotation failing to open its next file, for reasons like a full disk) is now also caught and falls back to the current file rather than throwing.
- Fixed a resource-exhaustion gap: since the wrapped server fully controls each topic string, it could open unbounded file handles and directories just by logging with a different topic every time (e.g. one per request id) - trivially reproduced with 500 distinct topics producing 500 open files and directories. Capped at 50 concurrent topic logs per run; beyond that, further new topics fall back to the main session log with a one-time warning instead of continuing to open more files.
